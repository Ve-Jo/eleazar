...
import { createMusicButtons } from "./musicButtons.ts";

type LocalizationStrings = Record<string, Record<string, unknown>>;
type TranslationResult = { translation?: string };
type TrackLike = any;
type PlayerLike = any;
type UserLike = any;
type GeneratedImageState = { buffer: Buffer };

// Define localization strings
const localization_strings: LocalizationStrings = {
  music: {
    player: {
      title: {
        en: "Now Playing",
...
(async () => {
  const localizationEntries = localization_strings as Record<
    string,
    Record<string, unknown>
  >;
  for (const [category, components] of Object.entries(localizationEntries)) {
    for (const [component, componentStrings] of Object.entries(components)) {
      await hubClient.registerLocalizations(
        category,
        component,
        componentStrings as any,
        true
      );
    }
  }
...
let lastGeneratedImage: GeneratedImageState | null = null;
...
export async function createOrUpdateMusicPlayerEmbed(
  ...
  const getAvatarUrl = (user: UserLike) => {
    if (!user)
      ...
      const buffer = (await generateImage(
        "MusicPlayer",
        {
          ...
          nextSongs: await Promise.all(
            player.queue.tracks.slice(0, 5).map(async (t: any) => ({
              title:
                t.info.title ||
                (
                  await hubClient.getTranslation(
  ...
  if (!lastGeneratedImage) {
    throw new Error("Music player image was not generated");
  }
...
function createProgressBar(current: number, total: number, length = 12) {

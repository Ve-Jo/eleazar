...
async function handleSetProfile(
  ...
  if (name) {
    ...
    if (sanitizedName !== null) {
      updateData.realName = sanitizedName;
    }
  }
  ...
  try {
    await (hubClient as any).updateUserProfile(guild.id, user.id, validation.sanitized);
...
async function handleResetProfile(
  ...
  try {
    ...
    let resetData: PersonalizationProfile = {};
    ...
    await (hubClient as any).updateUserProfile(guild.id, user.id, resetData);

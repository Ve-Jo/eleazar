...
export default {
  ...
  async generateWithNanoGPT(
    ...
    const userModels = await hubClientAny.getAIHubModels(
      null,
      false,
      userId,
      "nanogpt",
    ...
    const availableModels = (userModels as AiModelLike[]).filter(
      (m) => m.id === modelId
    );
    ...
    const hubConfig = await hubClientAny
      .getAIMetrics()
      .catch(() => ({ nanogpt: {} }));
  ...
  async generateWithGradio(
    ...
    const output: any = await client.predict("/infer", [
      prompt,
      seed,
      !seed,
      width,

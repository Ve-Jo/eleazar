...
const command = {
  data: (): SlashCommandBuilder => {
    const builder = new SlashCommandBuilder()
      .setName("settings")
      .setDescription("Configure bot settings (level roles, etc.)")
      ...
      .addSubcommandGroup((group) =>
        group
          ...
          .addSubcommand((subcommand) =>
            subcommand
              .setName("add")
              .setDescription("Assign a role to be awarded at a specific chat level")
      ...
      if (subcommandGroup === "levelroles") {
        if (subcommand === "add") {
          ...
          try {
            ...
            if (typedError.message.includes("already assigned to level")) {
              if (typedError.message.includes("A different role")) {
                ...
                const match = typedError.message.match(/level (\d+)/);
                const existingLevel = match?.[1] || "unknown";
                await interaction.editReply(
            ...
            } else if (typedError.message.includes("level must be at least 1")) {
              await interaction.editReply(await i18n.__(`${i18nBaseKey}.error_level_invalid`));
            } else {
              throw dbError;
            }
          }
          ...
          for (const levelRole of levelRoles) {
            const roleMention = `<@&${levelRole.roleId}>`;
            description += `${String(

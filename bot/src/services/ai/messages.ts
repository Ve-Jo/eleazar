...
const lastUserComponentMessages = new Map<string, any>();
...
export function splitMessage(message: string, maxLength = 2000): string[] {
  ...
  if (inCodeBlock && lastChunk && !lastChunk.trim().endsWith("```")) {
...
async function buildModelOptions(models: any[], selectedModel: string | null | undefined, locale = "en"): Promise<ModelOption[]> {
...
export async function buildProviderOptions(availableModels: any[], locale = "en"): Promise<ModelOption[]> {
  const providers = ["nanogpt", "groq", "openrouter"];
  const counts: Record<string, number> = { nanogpt: 0, groq: 0, openrouter: 0 };
...
export async function buildPaginatedModelMenu(
  ...
  menu.addOptions(finalOptions as any);
...
export async function buildInteractionComponents(
  ...
  if (noButtons && availableModels?.length) {
    ...
    providerMenu.addOptions(providerOpts as any);
  ...
  if (!noButtons) {
    ...
    options.push({
      label: await translate("events.ai.buttons.menus.settingsSelect.tools"),
      description: prefs.toolsEnabled
        ? await translate("events.ai.buttons.menus.settingsSelect.toolsEnabled")
        : await translate("events.ai.buttons.menus.settingsSelect.toolsDisabled"),
      value: "tools",
    ...
    /*if (isOpenRouterModel) {
      ...
      options.push({
        label:
          await translate("events.ai.buttons.menus.settingsSelect.webSearch") ||
          "Web Search",
    ...
    options.push({
      label: await translate("events.ai.buttons.systemPrompt.clearContext", {
        current,
    ...
    options.push({
      label: await translate("events.ai.buttons.finetune.buttonLabel", locale),
    ...
    settingsMenu.addOptions(options as any);
...
export async function buildErrorComponents(
  ...
  const errorMenu = new StringSelectMenuBuilder()
    ...
    .setPlaceholder(
      (await translate("events.ai.errorMenu.placeholder", locale)) ||
        "Fix this error..."
    );
  ...
  // Retry option
  options.push({
    label: (await translate("events.ai.errorMenu.retry", locale)) || "Retry",
    description:
      (await translate("events.ai.errorMenu.retryDescription", locale)) ||
  ...
  options.push({
    label:
      (await translate("events.ai.errorMenu.switchModel", locale)) ||
      "Switch Model",
    description:
      (await translate("events.ai.errorMenu.switchModelDescription", locale)) ||
  ...
  options.push({
    label:
      (await translate("events.ai.errorMenu.settings", locale)) || "Settings",
    description:
      (await translate("events.ai.errorMenu.settingsDescription", locale)) ||
  ...
  options.push({
    label:
      (await translate("events.ai.errorMenu.clearContext", locale)) ||
      "Clear Context",
    description:
      (await translate("events.ai.errorMenu.clearContextDescription", locale)) ||
  ...
  errorMenu.addOptions(options as any);
...
export async function sendResponse(
  ...
  const sanitized = String(content ?? "")
    .replace(
      /<@[!&]?\d+>/g,
      await translate("events.ai.buttons.sanitization.mention", locale)
    )
    .replace(
      /@everyone/gi,
      await translate("events.ai.buttons.sanitization.everyone", locale)
    )
    .replace(
      /@here/gi,
      await translate("events.ai.buttons.sanitization.here", locale)
    );
...
export async function handleFinetuneModal(interaction: any, userId: string, locale = "en"): Promise<void> {
  const prefs = getUserPreferences(userId);
  ...
  if (prefs.selectedModel) {
    ...
    if (parts.length >= 2) {
      provider = parts[0] || provider; // First part is the provider
    }
  }
  ...
  const row = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder(
        (await translate("events.ai.buttons.finetune.selectParameter", locale)) ||
          "Select parameter to adjust"
      )
      .addOptions(paramOptions as any)
  );
  ...
  collector.on("collect", async (i: any) => {
    try {
      ...
      // Add input to modal
      modal.addComponents(new ActionRowBuilder<any>().addComponents(paramInput as any) as any);
      ...
      try {
        await i
          .reply({
            content: await translate("events.ai.buttons.finetune.error", locale),
            ephemeral: true,
          })
          .catch(() => {});
...
function validateParameterValue(input: string, paramName: string, currentValue: any, paramConfig: any): number {
...
function getReasoningEmoji(level: string | null | undefined): string {
